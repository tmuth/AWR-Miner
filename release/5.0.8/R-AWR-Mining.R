...
#parseOverride <- "ALL" [ALL|NONE|SOME|PAGE1|AAS] | rm(parseOverride) | parseOverride <- c("SOME","aas_facet")
...
add_missing_dates <- function(DF_SNAP_ID_DATE_INT){
  ...
  all.dates <- data.frame(end=seq(time.min, time.max, by="day"))
  
  existing.dates <- data.frame(end=unique(floor_date(DF_SNAP_ID_DATE_INT$end,"day")))
  
  
  missing.dates <- anti_join(all.dates,existing.dates,by="end")
  
  if(nrow(missing.dates)==0){
    return(DF_SNAP_ID_DATE_INT)
  }
  missing.dates <- data.frame(SNAP_ID=NA,end=missing.dates$end,dur_m=NA)
  
  missing.dates2 <- missing.dates
  missing.dates$end <- missing.dates$end + minutes(1)
  missing.dates2$end <- missing.dates2$end + hours(23)
  missing.dates <- rbind(missing.dates,missing.dates2)
  DF_SNAP_ID_DATE_INT.full <- rbind(DF_SNAP_ID_DATE_INT,data.table(missing.dates))
  
  DF_SNAP_ID_DATE_INT.full <- DF_SNAP_ID_DATE_INT.full %>%
    arrange(end) %>%
...
build_data_frames <- function(fileName) {
  ...
  # do this to then count decimals to see if decimal or comma are the separator 
...
plot_aas_chart <- function(DF_AAS_INT){
  ...
  vals <- expand.grid(end = unique(main$DF_SNAP_ID_DATE$end),
                      WAIT_CLASS = unique(DF_AAS_INT$WAIT_CLASS))
  vals <- data.table(vals)
  DF_AAS_INT <- merge(DF_AAS_INT,vals,by=c("end","WAIT_CLASS"),all.y=TRUE)
...
plot_IO_by_object_type <- function(DF_IO_BY_OBJECT_TYPE_INT){
  
  flog.debug('plot_IO_by_object_type - start',name='plot_IO_by_object_type')
  
  DF_IO_BY_OBJECT_TYPE_INT <- dplyr::left_join(DF_IO_BY_OBJECT_TYPE_INT,main$DF_SNAP_ID_DATE,by="SNAP_ID")
  
  io.melt <- melt(DF_IO_BY_OBJECT_TYPE_INT,d.var = c("end","OBJECT_TYPE"), measure.var = c("LOGICAL_READ_GB", "PHYSICAL_READ_GB", "PHYSICAL_WRITE_GB", "GB_ADDED"))
  
  io.melt$variable <-  gsub( "LOGICAL_READ_GB" , "Logical Reads GB" , io.melt$variable)
  io.melt$variable <-  gsub( "PHYSICAL_READ_GB" , "Physical Reads GB" , io.melt$variable)
  io.melt$variable <-  gsub( "PHYSICAL_WRITE_GB" , "Physical Writes GB" , io.melt$variable)
  io.melt$variable <-  gsub( "GB_ADDED" , "GB Added" , io.melt$variable)
  
  io.melt <- io.melt %>%
    filter(value > 0)
  
  vals <- expand.grid(end = unique(main$DF_SNAP_ID_DATE$end),
                      OBJECT_TYPE = unique(io.melt$OBJECT_TYPE),
                      variable = unique(io.melt$variable))
  
  
  io.melt <- merge(io.melt,vals,by=c("end","OBJECT_TYPE","variable"),all.y=TRUE)
  io.melt$value <- ifelse(is.na(io.melt$value),0,io.melt$value)
  
  
  
  
  max_vals <- ddply(io.melt, .(variable,OBJECT_TYPE,format(end,"%y/%m/%d")), subset, subset = rank(-value) <= 1)
  ...
  p <- ggplot() +
    ...
    xlim(min(io.melt$end),max(io.melt$end))+
    theme(text =               element_text(size = 6))+
    labs(title=paste("Segment Activity for ",main$current_db_name,sep=""))+
    main$gg_hour_bars+
    attr$vertical_line + attr$vertical_text +
    scale_x_datetime(labels = date_format_tz("%a, %b %d %I %p", tz="UTC"),breaks = attr$date_break_major_var,
                     minor_breaks = attr$date_break_minor_var,
                     limits = c(min(io.melt$end),max(io.melt$end))
    )+
    guides(fill=FALSE) 
  ...
  io.melt.ext.group <- io.melt %>%
    filter(value > 0) %>%
    group_by(variable,OBJECT_TYPE) %>%
  ...
  io.melt.ext.group$variable <-  gsub( " GB$" , "" , io.melt.ext.group$variable)
  io.melt.ext.group$variable <-  gsub( "^GB Added$" , "Data Growth" , io.melt.ext.group$variable)
  
  
  io_obj_type_pct_plot <- ggplot(io.melt.ext.group,aes(x=OBJECT_TYPE,fill = OBJECT_TYPE))+
    geom_bar(stat ='identity',aes(y=pct),width=1,alpha=.8)+
    geom_text(aes(label=round(pct*100,0),y=pct),size=2)+
    facet_grid(. ~ variable,scales="free_y")+
    scale_y_continuous(labels = percent_format())+
    attr$themeScaleFill+
    ...
    labs(title=paste("Segment Activity Percent for ",main$current_db_name,sep=""))
  
  
  flog.debug('plot_IO_by_object_type - end',name='plot_IO_by_object_type')
  return(list(io_obj_type_pct_plot,p))
}
...
plot_io <- function(DF_MAIN_BY_SNAP_INT){
  ...
  x.melt <- melt(DF_MAIN_BY_SNAP_INT, id.var = c("end"), measure.var = c("read_iops", "read_iops_max","write_iops", "write_iops_max",
  ...
  vals <- data.table(vals)
  x.melt  <- merge(x.melt ,vals,by=c("end","variable"),all.y=TRUE)
...
plot_io_histograms <- function(DF_IO_WAIT_HIST_INT){
  ...
  vals <- left_join(tbl_dt(vals),end.fill,copy = TRUE,by=c("end"))
...
plot_iostat_by_function <- function(DF_IOSTAT_FUNCTION_INT){
  ...
  #DF_IOSTAT_FUNCTION_INT2 <- merge(DF_IOSTAT_FUNCTION_INT,main$DF_SNAP_ID_SUBSET,by="SNAP_ID")
  
  iostat2.melt <- melt(DF_IOSTAT_FUNCTION_INT,id.var = c("SNAP_ID","FUNCTION_NAME"),measure.var = c("SM_R_REQS","SM_W_REQS", "LG_R_REQS", "LG_W_REQS"))
  iostat2.melt <- merge(iostat2.melt,main$DF_SNAP_ID_DATE,by="SNAP_ID")
  
  #iostat2.melt.tmp <<- iostat2.melt 
  
  
  vals <- expand.grid(end = unique(main$DF_SNAP_ID_DATE$end),
                      FUNCTION_NAME = unique(iostat2.melt$FUNCTION_NAME),
                      variable = unique(iostat2.melt$variable)
  )
  
  vals <- merge(vals,main$DF_SNAP_ID_DATE,by="end")
  
  #end.fill <- unique(iostat2.melt %>% select(SNAP_ID,end))
  #vals <- left_join(tbl_dt(vals),end.fill,copy = TRUE,by=c("end"))
  #iostat2.melt <- left_join(tbl_dt(vals),iostat2.melt,copy = TRUE)
  iostat2.melt  <- merge(iostat2.melt ,vals,by=c("end","SNAP_ID","FUNCTION_NAME","variable","dur_m"),all.y=TRUE)
  
  
  
  #iostat2.melt[is.na(iostat2.melt$value)]$value <- 0
  iostat2.melt$value <- ifelse(is.na(iostat2.melt$value),0,iostat2.melt$value)
...
plot_cpu_sd <- function(DF_MAIN_INT){
  ...
  DF_MAIN_INT$cpu_per_s_pct <- round((DF_MAIN_INT$cpu_per_s / get_os_stat("NUM_CPUS"))*100,1)
  DF_MAIN_INT$cpu_per_s_sd_pct <- round((DF_MAIN_INT$cpu_per_s_sd / get_os_stat("NUM_CPUS"))*100,1)
...
plot_main_activity <- function(DF_MAIN_INT){
  ...
  DF_SNAP_ID_SUBSET2 <- merge(main$DF_SNAP_ID_SUBSET,vals,by="end")
...
plot_sql_aggregates <- function(DF_SQL_BY_SNAPID_IN){
  flog.debug('plot_sql_aggregates - start')
  
  top_n_schema_filter <- 10
  ...
  DT_SQL_BY_SNAPID <- data.table(DF_SQL_BY_SNAPID_IN)
  ...
  DT_SQL_BY_SNAPID$LOG_READS <- cleanStringNumbers(DT_SQL_BY_SNAPID$BUFFER_GETS)
  ...
  DT_SQL_BY_SNAPID_GRP_BY_COMMAND <- data.table(DT_SQL_BY_SNAPID[, list(ELAP_S = sum(ELAP_S),EXECS=sum(EXECS),
                                                                        #cost=max(OPTIMIZER_COST),
                                                                        IO_WAIT=sum(IO_WAIT),READ_MB=sum(READ_MB),CPU_T_S=sum(CPU_T_S),
  ...
  #DT_SQL_BY_SNAPID_GRP_BY_COMMAND_OUT <<- DT_SQL_BY_SNAPID_GRP_BY_COMMAND
  ...
  sql_pct_by_cmd_plot <- ggplot(data=DT_SQL_BY_SNAPID_GRP_BY_COMMAND.melt, aes(x=COMMAND, y=value),aes(color=COMMAND)) +
    geom_bar(stat="identity",aes(fill=COMMAND))+
    geom_text(aes(label=round(value*100,1)),size=2.5, vjust=0.5, hjust=0)+
    #facet_grid(variable ~ .,scales="free_y")+
    facet_grid(variable ~ .)+
    scale_y_continuous(labels = percent_format())+
    theme(legend.position="none",axis.title.x  = element_blank(),axis.title.y  = element_blank(),
          axis.text.x=element_text(angle=-30),strip.text.y = element_text(size = 5))+
    labs(title="SQL Percent by Command by Metric for All SQL")+
    scale_fill_stata()
  
  
  
  
  DT_SQL_TOP_N_SCHEMAS <- DT_SQL_BY_SNAPID %>%
    group_by(SCHEMA) %>%
    summarize(ELAP_S = sum(ELAP_S)) %>%
    arrange(desc(ELAP_S)) %>%
    mutate(rank = dense_rank(desc(ELAP_S))) %>%
    filter(rank <= top_n_schema_filter)
  
  
  
  DT_SQL_BY_SNAPID.schemas <- semi_join(DT_SQL_BY_SNAPID,DT_SQL_TOP_N_SCHEMAS,by="SCHEMA")
  
  
  
  DT_SQL_BY_SNAPID_GRP_BY_SCHEMA <- data.table(DT_SQL_BY_SNAPID.schemas[, list(ELAP_S = sum(ELAP_S),EXECS=sum(EXECS),
                                                                               #cost=max(OPTIMIZER_COST),
                                                                               IO_WAIT=sum(IO_WAIT),READ_MB=sum(READ_MB),CPU_T_S=sum(CPU_T_S),
                                                                               ELAP_P_EXEC=(sum(ELAP_S)/sum(EXECS))),
                                                                        by = list(SCHEMA)])
  DT_SQL_BY_SNAPID_GRP_BY_SCHEMA$Executions <- DT_SQL_BY_SNAPID_GRP_BY_SCHEMA$EXECS / sum(DT_SQL_BY_SNAPID_GRP_BY_SCHEMA$EXECS)
  DT_SQL_BY_SNAPID_GRP_BY_SCHEMA$"Elapsed Time" <- DT_SQL_BY_SNAPID_GRP_BY_SCHEMA$ELAP_S / sum(DT_SQL_BY_SNAPID_GRP_BY_SCHEMA$ELAP_S)
  DT_SQL_BY_SNAPID_GRP_BY_SCHEMA$"I/O Wait" <- DT_SQL_BY_SNAPID_GRP_BY_SCHEMA$IO_WAIT / sum(DT_SQL_BY_SNAPID_GRP_BY_SCHEMA$IO_WAIT)
  DT_SQL_BY_SNAPID_GRP_BY_SCHEMA$READ_MB_PCT <- DT_SQL_BY_SNAPID_GRP_BY_SCHEMA$READ_MB / sum(DT_SQL_BY_SNAPID_GRP_BY_SCHEMA$READ_MB)
  DT_SQL_BY_SNAPID_GRP_BY_SCHEMA$"CPU Time" <- DT_SQL_BY_SNAPID_GRP_BY_SCHEMA$CPU_T_S / sum(DT_SQL_BY_SNAPID_GRP_BY_SCHEMA$CPU_T_S)
  DT_SQL_BY_SNAPID_GRP_BY_SCHEMA$"Elap Per Exec" <- DT_SQL_BY_SNAPID_GRP_BY_SCHEMA$ELAP_P_EXEC / sum(DT_SQL_BY_SNAPID_GRP_BY_SCHEMA$ELAP_P_EXEC)
  
  #print(head(DT_SQL_BY_SNAPID_GRP_BY_SCHEMA))
  
  DT_SQL_BY_SNAPID_GRP_BY_SCHEMA.melt <- melt(DT_SQL_BY_SNAPID_GRP_BY_SCHEMA, id.var = c("SCHEMA"), 
                                              measure.var = c("Elapsed Time","I/O Wait","Elap Per Exec"))
  
  sql_pct_by_cmd_by_schema_plot <- ggplot(data=DT_SQL_BY_SNAPID_GRP_BY_SCHEMA.melt, aes(x=SCHEMA, y=value),aes(color=SCHEMA)) +
    geom_bar(stat="identity",aes(fill=SCHEMA))+
    geom_text(aes(label=round(value*100,1)),size=2.5, vjust=0.5, hjust=0)+
    facet_grid(variable ~ .,scales="free_y")+
    scale_y_continuous(labels = percent_format())+
    theme(legend.position="none",axis.title.x  = element_blank(),axis.title.y  = element_blank(),
          axis.text.x=element_text(angle=-30),strip.text.y = element_text(size = 5))+
    labs(title=paste0("SQL Percent by Schema by Metric (top ",top_n_schema_filter," schemas)"))+
    scale_fill_stata()
  ...
  brks <- as.vector(c(0.1,0.25,1,10,60,600,3600,21600)) #establish bins for SQL
  ...
  #DT_SQL_BY_SNAPID_OUT <<- DT_SQL_BY_SNAPID
  
  DT_SQL_GROUP_BY_BIN <- data.table(DT_SQL_BY_SNAPID[, list(ELAP_S = sum(ELAP_S)),
                                                     by = list(bin,bin2)])
  DT_SQL_GROUP_BY_BIN <- DT_SQL_GROUP_BY_BIN[,Elapsed_Pct:=ELAP_S/sum(ELAP_S)][]
  
  DT_SQL_GROUP_BY_BIN$bin3 <- reorder(DT_SQL_GROUP_BY_BIN$bin2, DT_SQL_GROUP_BY_BIN$bin)
  
  hist_sql_plot <- ggplot(data=DT_SQL_GROUP_BY_BIN, aes(x=bin3, y=Elapsed_Pct),aes(color=bin3)) +
    geom_bar(stat="identity",aes(fill=bin3))+
    geom_text(aes(label=round(Elapsed_Pct*100,1)),size=2.5, vjust=0.5, hjust=0)+
    #facet_grid(variable ~ .,scales="free_y")+
    scale_y_continuous(labels = percent_format())+
    xlab("Bins - Time in seconds")+
    theme(legend.position="none",axis.title.y  = element_blank(),
          axis.text.x=element_text(angle=-30),strip.text.y = element_text(size = 5))+
    labs(title="Histogram of Aggregate SQL Elapsed Time Per Execution")+
    scale_fill_stata()
  ...
  DT_SQL_GROUP_BY_SQLID_by_SCHEMA <- data.table(DT_SQL_BY_SNAPID.schemas[, list(ELAP_S = sum(ELAP_S)),
                                                                         by = list(SCHEMA,bin,bin2)])
  
  DT_SQL_GROUP_BY_SQLID_by_SCHEMA <- semi_join(DT_SQL_GROUP_BY_SQLID_by_SCHEMA,DT_SQL_TOP_N_SCHEMAS,by="SCHEMA")
  
  #   
  DT_SQL_GROUP_BY_SQLID_by_SCHEMA <- DT_SQL_GROUP_BY_SQLID_by_SCHEMA[,Elapsed_Pct:=ELAP_S/sum(ELAP_S),by=SCHEMA][]
  #   
  DT_SQL_GROUP_BY_SQLID_by_SCHEMA$bin3 <- reorder(DT_SQL_GROUP_BY_SQLID_by_SCHEMA$bin2, DT_SQL_GROUP_BY_SQLID_by_SCHEMA$bin)
  #   
  hist_sql_by_schema_plot <- ggplot(data=DT_SQL_GROUP_BY_SQLID_by_SCHEMA, aes(x=bin3, y=Elapsed_Pct),aes(color=bin3)) +
    geom_bar(stat="identity",aes(fill=bin3))+
    geom_text(aes(label=round(Elapsed_Pct*100,1)),size=1.5, vjust=0.5, hjust=0)+
    facet_grid(SCHEMA ~ .,scales="free_y")+
    scale_y_continuous(labels = percent_format())+
    xlab("Bins - Time in seconds")+
    theme(legend.position="none",axis.title.y  = element_blank(),
          axis.text.x=element_text(angle=0),strip.text.y = element_text(size = 3),
          axis.text.y=element_text(size = 4))+
    labs(title=paste0("Histogram of SQL Elapsed Time Per Execution by Schema (top ",top_n_schema_filter," schemas)"))+
    scale_fill_stata()
  #   
  return(list(sql_pct_by_cmd_plot,sql_pct_by_cmd_by_schema_plot,hist_sql_plot,hist_sql_by_schema_plot))
  flog.debug('plot_sql_aggregates - end')
}
...
main$mainFunction <- function(f){
  ...
  if(okToPrintPlot('io_by_object_type')){ 
    
    c(io_by_obj_type_pct_plot, io_by_obj_type_plot) := plot_IO_by_object_type(main$DF_IO_BY_OBJECT_TYPE)
    flog.debug('io_by_object_type - start',name='print')
    x <- grid.arrange(io_by_obj_type_pct_plot,io_by_obj_type_plot , ncol = 1, heights=c(1,4),as.table=FALSE)
    #x <- grid.arrange(io_hist_plot, ncol = 1)
    #x <- grid.arrange(io_hist_area_plot, ncol = 1)
    flog.debug('io_by_object_type - end',name='print')
  ...
  if(okToPrintPlot('sql_aggregates')){
    if(nrow(main$DF_SQL_BY_SNAPID)> 10){
      ...
      tryCatch({
        ...
        flog.debug('print_sql_aggregates_plot - start',name='print')
