...
with top_n_sql_ids as (select snap_id,sql_id from(
select snap_id,sql_id,
DENSE_RANK() OVER
      (partition by snap_id ORDER BY sum(ELAPSED_TIME_DELTA) DESC ) elap_rank
