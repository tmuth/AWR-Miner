...
begin
  ...
  if :DB_VERSION_1  >= 12.1 then
		l_variables := l_variables||',ver_gte_12_1:TRUE';
	else
		l_variables := l_variables||',ver_gte_12_1:FALSE';
	end if;

  if :DB_VERSION_1  >= 12.2 then
    l_variables := l_variables||',ver_gte_12_2:TRUE';
  else
    l_variables := l_variables||',ver_gte_12_2:FALSE';
  end if;
...
BEGIN
  ...
  for c6 in (SELECT $IF $$VER_GTE_12_1 $THEN distinct cdb FROM sys.GV_$DATABASE WHERE dbid = &DBID and rownum = 1 $ELSE 'NO' CDB FROM DUAL $END)
	loop
		dbms_output.put_line(rpad('CONTAINER_DATABASE',l_pad_length)||' '||c6.cdb);

      if c6.cdb = 'YES' then
        FOR c7 IN ($IF $$VER_GTE_12_1 $THEN SELECT COUNT(*) NO_PDBS FROM V$PDBS $ELSE SELECT 0 NO_PDBS FROM DUAL $END)
        loop
          dbms_output.put_line(rpad('NUMBER_PDBS',l_pad_length)||' '||c7.no_pdbs);
        end loop;
      end if;
	end loop;
...
SELECT SNAP_ID,
  ...
  ROUND(MAX(DECODE(event_name,'cell physical IO bytes saved by storage index', event_val_diff,NULL))             /1024/1024,1) "cell_int_si_mb",
  ROUND(MAX(DECODE(event_name,'cell physical IO bytes eligible for predicate offload', event_val_diff,NULL))             /1024/1024,1) "cell_elig_offload",
  ROUND(MAX(DECODE(event_name,'cell IO uncompressed bytes', event_val_diff,NULL))             /1024/1024,1) "cell_uncomp_mb",
  ROUND(MAX(DECODE(event_name,'IM scan rows', event_val_diff,NULL))             ) "im_scan_rows"
FROM
  (SELECT snap_id,
    event_name,
  ...
  FROM
    ...
    FROM
      ...
      'physical read total bytes','physical read total bytes optimized','cell physical IO bytes eligible for predicate offload','cell IO uncompressed bytes','IM scan rows')
      )
